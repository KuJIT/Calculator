//Программа, разбивающая математическое выражение на дерево выражений (определены круглые скобки, +,-,*,/ и десятичные дроби)
//
//из проблем:
//Класс очень тяжело расширяется, особенно для введния унарных операций. Стоило абстрагировать операцию разбиения выражения
...
//Однако обрабатывает выражение типа "-2"(сделано при помощи костыля, который работает только с константами)
...
//По-моему, у класса получился неплохой интерфейс - Конструктор для создания объекта и метод Compute() и выбрасывание исключения
//при неправильном формате входной строки. Стоило ещё оставить
